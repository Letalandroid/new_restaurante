<script setup lang="ts">
import { ref } from 'vue';
import axios, { AxiosError } from 'axios';
import Dialog from 'primevue/dialog';
import Toolbar from 'primevue/toolbar';
import Button from 'primevue/button';
import InputText from 'primevue/inputtext';
import Calendar from 'primevue/calendar';
import Checkbox from 'primevue/checkbox';
import { useToast } from 'primevue/usetoast';

interface Holiday {
    name: string;
    date: string | Date | null;
    is_recurring: boolean;
}

interface ServerErrors {
    [key: string]: string[];
}

const toast = useToast();
const submitted = ref<boolean>(false);
const holidayDialog = ref<boolean>(false);
const serverErrors = ref<ServerErrors>({});
const emit = defineEmits<{
    (e: 'holiday-agregado'): void;
}>();

const holiday = ref<Holiday>({
    name: '',
    date: null,
    is_recurring: false,
});

function resetHoliday() {
    holiday.value = {
        name: '',
        date: null,
        is_recurring: false,
    };
    serverErrors.value = {};
    submitted.value = false;
}

function openNew() {
    resetHoliday();
    holidayDialog.value = true;
}

function hideDialog() {
    holidayDialog.value = false;
    resetHoliday();
}

async function guardarHoliday() {
    submitted.value = true;
    serverErrors.value = {};

    if (!holiday.value.name || !holiday.value.date) {
        toast.add({ severity: 'warn', summary: 'Campos obligatorios', detail: 'Completa los campos requeridos.', life: 3000 });
        return;
    }

    try {
        await axios.post('/asistencias/feriado', {
            name: holiday.value.name,
            date: holiday.value.date,
            is_recurring: holiday.value.is_recurring,
        });

        toast.add({ severity: 'success', summary: 'Éxito', detail: 'Feriado registrado correctamente', life: 3000 });
        hideDialog();
        emit('holiday-agregado');
    } catch (error) {
        const err = error as AxiosError;
        if (err.response && err.response.status === 422) {
            serverErrors.value = (err.response.data as any).errors || {};
        } else {
            toast.add({
                severity: 'error',
                summary: 'Error',
                detail: 'No se pudo registrar el feriado',
                life: 3000,
            });
        }
    }
}
</script>

<template>
    <Toolbar class="mb-6">
        <template #start>
            <Button label="Nuevo feriado" icon="pi pi-plus" severity="secondary" class="mr-2" @click="openNew" />
        </template>
    </Toolbar>

    <Dialog v-model:visible="holidayDialog" :style="{ width: '600px' }" header="Registro de feriados" :modal="true">
        <div class="flex flex-col gap-6">
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-8">
                    <label for="name" class="block font-bold mb-3">Nombre del feriado <span class="text-red-500">*</span></label>
                    <InputText id="name" v-model.trim="holiday.name" required maxlength="100" fluid />
                    <small v-if="submitted && !holiday.name" class="text-red-500">El nombre es obligatorio.</small>
                    <small v-else-if="serverErrors.name" class="text-red-500">{{ serverErrors.name[0] }}</small>
                </div>

                <div class="col-span-4">
                    <label for="date" class="block font-bold mb-3">Fecha <span class="text-red-500">*</span></label>
                    <Calendar id="date" v-model="holiday.date" dateFormat="yy-mm-dd" showIcon />
                    <small v-if="submitted && !holiday.date" class="text-red-500">La fecha es obligatoria.</small>
                    <small v-else-if="serverErrors.date" class="text-red-500">{{ serverErrors.date[0] }}</small>
                </div>

                <div class="col-span-12 flex items-center gap-3 mt-3">
                    <Checkbox v-model="holiday.is_recurring" :binary="true" inputId="is_recurring" />
                    <label for="is_recurring" class="font-bold">Se repite cada año</label>
                </div>
            </div>
        </div>

        <template #footer>
            <Button label="Cancelar" icon="pi pi-times" text @click="hideDialog" />
            <Button label="Guardar" icon="pi pi-check" @click="guardarHoliday" />
        </template>
    </Dialog>
</template>
